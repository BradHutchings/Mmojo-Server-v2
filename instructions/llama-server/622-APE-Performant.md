...
- Run the script.
  ```
  $MMOJO_SERVER_SCRIPTS/620-Create-Package-APE.sh performant attired llama-server
...
- Run the script.
  ```
  $MMOJO_SERVER_SCRIPTS/620-Add-Certificates-to-APE.sh performant attired llama-server
...
- Run the script.
  ```
  $MMOJO_SERVER_SCRIPTS/620-Add-UI-to-APE.sh performant attired llama-server
  # Keep track of what we add for the args file.
...
- Run the script.
  ...
  if [ -v CHOSEN_MODEL ]; then
    $MMOJO_SERVER_SCRIPTS/620-Add-gguf-Model-to-APE.sh performant attired llama-server
    # Keep track of what we add for the Args file.
...
- Run the script.
  ```
  $MMOJO_SERVER_SCRIPTS/620-Add-Args-to-APE.sh performant attired llama-server
  ```
...
```
THIS_PACKAGE_DIR="$LLAMA_SERVER_PACKAGE_DIR/$PACKAGE_PERFORMANT_APE"
APP_FILE="$PACKAGE_LLAMA_SERVER_APE_FILE-perf"
...
- Run the script.
  ```
  $MMOJO_SERVER_SCRIPTS/620-Copy-APE-Package-to-Mmojo-Share.sh performant
  ```
-->
