...
rm -r -f $BUILD_DIR/$BUILD_VULKAN
cmake -B $BUILD_VULKAN -DBUILD_SHARED_LIBS=OFF -DLLAMA_CURL=OFF -DLLAMA_OPENSSL=ON \
    -DCMAKE_BUILD_TYPE=Release -DGGML_NATIVE=ON -DGGML_VULKAN=ON # -DCMAKE_VERBOSE_MAKEFILE=ON 
cmake --build $BUILD_VULKAN

# Show off what we built
printf "\nBuild of llama.cpp with Vulkan support is complete.\n\n"
printf "\$ ls -al $BUILD_DIR/$BUILD_VULKAN/bin/\n"
ls -al $BUILD_DIR/$BUILD_VULKAN/bin
printf "\n"
