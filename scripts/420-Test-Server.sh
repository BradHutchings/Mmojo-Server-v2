...
if [ $processor == "x86_64" ]; then
    BUILD_SUBDIRECTORY="$BUILD_EXECUTABLECOMPATIBLE_X86_64"
    if [ $variation == "performant" ]; then
        BUILD_SUBDIRECTORY="$BUILD_EXECUTABLEPERFORMANT_X86_64"
    elif [ $variation == "native" ]; then
        BUILD_SUBDIRECTORY="$BUILD_EXECUTABLENATIVE_X86_64"
    fi
fi
if [ $processor == "aarch64" ]; then
    BUILD_SUBDIRECTORY="$BUILD_EXECUTABLECOMPATIBLE_AARCH64"
    if [ $variation == "performant" ]; then
        BUILD_SUBDIRECTORY="$BUILD_EXECUTABLEPERFORMANT_AARCH64"
    elif [ $variation == "native" ]; then
        BUILD_SUBDIRECTORY="$BUILD_EXECUTABLENATIVE_AARCH64"
    fi
fi
