...
THIS_TEST="$TEST_DIR/$TEST_VULKAN"
mkdir -p $THIS_TEST
rm -r -f $THIS_TEST/*
cd $THIS_TEST
...
# --mlock is not needed to run this, even with VULKAN GPU support.
$BUILD_DIR/$BUILD_VULKAN/bin/mmojo-server --model $MODELS_DIR/$MODEL_PARAM \
