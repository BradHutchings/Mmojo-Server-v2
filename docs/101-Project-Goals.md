## 101. Project Goals
### History
Mmojo Server was inspired by Mozilla's llamafile, which built llama.cpp as an Actual Portable Executable (APE) file. This file could run on x86 and ARM, under Linux, macOS, Windows, FreeBSD, and a few other less notable operating systems. The APE file contained the LLM, often gigabyes in size. It was download once, run anywhere. I shipped llamafile APE files that also included my Mmojo Completion user interface and incorporated SSL certificates. I encouraged my consulting clients to use these on their PCs rather than using cloud LLMs. Even though they aren't very technical people, they were mostly able to make llamafiles work because they didn't have to do too much. Comparatively, systems like Ollama &mdash; built for non-technical people with an extensive UI &mdash; were initimidating!

Unfortunately, Mozilla stopped actively supporting llamafile around the fall of 2024. This meant that support for new models rolled out in llama.cpp was not incorporated into llamafile. This turned out to be the greatest weakness of llamafile. If nobody kept it up to date with the additions to and changes in llama.cpp, it just wasn't that interesting. This turned out to be the main organizing principle for Mmojo Server. It had to offer the run everywhere feature of llamafile, but had to be maintainable, especially the llama.cpp part.
